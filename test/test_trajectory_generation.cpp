...
TrajectoryGenerator* buildFigure8() {
...
VectorXd* getFigure8Solution(long size) {
  VectorXd* expected = new VectorXd[3];
  expected[0] = VectorXd(size);
  expected[1] = VectorXd(size);
  expected[2] = VectorXd(size);
...
TEST(traj_generation, figure8_linear) {
    TrajectoryGenerator* tg = buildFigure8();

    tg->solveProblem(TrajectoryGenerator::Solver::LINEAR_SOLVE);
    VectorXd* expected = getFigure8Solution(tg->getNumVars());

    for(int i = 0; i < 3; ++i) {
      VectorXd solution = tg->getSolution(i);
      ASSERT_TRUE(solution.isApprox(expected[i]));
    }
...
TEST(traj_generation, figure8_bleic) {
  TrajectoryGenerator* tg = buildFigure8();
  ...
  for(int i = 0; i < 3; ++i) {
    VectorXd solution = tg->getSolution(i);
    ASSERT_TRUE(solution.isApprox(expected[i], 1e-6));
  }
...
#ifdef USE_OOQP
TEST(traj_generation, figure8_OOQP) {
  TrajectoryGenerator* tg = buildFigure8();
...
TrajectoryGenerator* buildSlalom() {
...
VectorXd* getSlalomSolution(long size) {
  VectorXd* expected = new VectorXd[3];
  expected[0] = VectorXd(size);
  expected[1] = VectorXd(size);
  expected[2] = VectorXd(size);
...
TEST(traj_generation, slalom_linear) {
  TrajectoryGenerator* tg = buildSlalom();
  VectorXd* expected = getSlalomSolution(tg->getNumVars());
  tg->solveProblem(TrajectoryGenerator::Solver::LINEAR_SOLVE);
  for(int i = 0; i < 3; ++i) {
    VectorXd solution = tg->getSolution(i);
    ASSERT_TRUE(solution.isApprox(expected[i]));
...
#ifdef USE_OOQP
TEST(traj_generation, slalom_ooqp) {
  TrajectoryGenerator* tg = buildSlalom();
  VectorXd* expected = getSlalomSolution(tg->getNumVars());
  tg->solveProblem(TrajectoryGenerator::Solver::OOQP);
  for(int i = 0; i < 3; ++i) {
...
TEST(traj_generation, slalom_BLEIC) {
  TrajectoryGenerator* tg = buildSlalom();
  VectorXd* expected = getSlalomSolution(tg->getNumVars());
  tg->solveProblem(TrajectoryGenerator::Solver::BLEIC);
  for(int i = 0; i < 3; ++i) {
